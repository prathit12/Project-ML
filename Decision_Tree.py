...
def run_decision_tree():
    ...
    # Query even and odd images from the MongoDB collection
    ...
    for k in range(K):
        ...
        # Split the dataset into training and test sets for cross-validation
        training_indices = [i for i in range(len(dataset)) if i % K != k]
        test_indices = [i for i in range(len(dataset)) if i % K == k]

        training_set = even_image_fd_list  # Use even images as the training set
        test_set = [even_image_fd_list[i] for i in test_indices]  # Use even images for testing
        ...
        root = compute_decision_tree(training_set, None, even_labels)

        # Classify the test set using the constructed tree
        results = [get_classification(instance, root, dataset.class_col_index) for instance in test_set]

        # Calculate accuracy
        acc = sum(result == instance[-1] for result, instance in zip(results, test_set)) / len(test_set)
